...
namespace UnrealBuildTool.Rules
{
	...
	PublicDependencyModuleNames.Add("DeveloperSettings");
			...
			/*
				...
				set flags for different plugins
				each plugin gets a function that takes this flag and the plugins directory
				easy way for devs to turn something on/off without messing with code/comments

			 */

			//plugin flags
			//set to true to look for specified plugin
			bool VarjoFlag = false;
			VarjoPlugin(VarjoFlag, pluginsDirectory);

			bool TobiiEyeTrackingFlag = false;
			TobiiEyeTrackingPlugin(TobiiEyeTrackingFlag, pluginsDirectory);

			bool ViveProEyeSRanipalFlag = false;
			ViveProEyeSRanipalPlugin(ViveProEyeSRanipalFlag, pluginsDirectory);

			bool PicoNeo2EyeFlag = false;
			PicoNeo2EyePlugin(PicoNeo2EyeFlag, pluginsDirectory);

			bool HPOmnicetFlag = false;
			HPOmniceptPlugin(HPOmnicetFlag, pluginsDirectory);
		...
		void VarjoPlugin(bool Flag, string PluginsDirectory)
        {
			if(Flag)
            {
		...
		//TobiiEyeTracking
		void TobiiEyeTrackingPlugin(bool Flag, string PluginsDirectory)
		{
			if (Flag)
			{
		...
		void ViveProEyeSRanipalPlugin(bool Flag, string PluginsDirectory)
		{
			if (Flag)
			{
		...
		//Pico Neo 2 Eye
		void PicoNeo2EyePlugin(bool Flag, string PluginsDirectory)
		{
			if (Flag)
			{
		...
		//HP Omnicept
		void HPOmniceptPlugin(bool Flag, string PluginsDirectory)
		{
			if (Flag)
			{
